class CompetitionPhoto < ActiveRecord::Base
  ...
  has_many :jury_ratings, through: :competition
  ...
  scope :with_nomination, -> (nom_id) { where(nomination_id: nom_id) }
  ...
  def average_rating(role=nil)
    jr = if role
           vips ||= User.with_role(role).map(&:id)
           jury_ratings.where(user_id: vips)
         else
           jury_ratings
         end
  ...
  def sd(role=nil)
    ...
    jr.extend(DescriptiveStatistics)
    jr.standard_deviation(&:rating).to_f.round(2)
  end
